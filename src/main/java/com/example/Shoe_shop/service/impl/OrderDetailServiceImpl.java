...
public class OrderDetailServiceImpl implements OrderDetailService {
    ...
    public OrderDetailView getOrderDetail(Long orderId, Long userId) {
        ...
        // Calculate subtotal (product total without shipping)
        BigDecimal subtotal = order.getTotalAmount().subtract(order.getShippingFee());
        // Total sum is already stored in totalAmount (includes shipping)
        BigDecimal totalSum = order.getTotalAmount();
        ...
        return new OrderDetailView(
                ...
                order.getCreatedAt(),
                subtotal,              // Tổng tiền sản phẩm (chưa bao gồm ship)
                order.getShippingFee(), // Phí vận chuyển
                totalSum,              // Tổng cộng (đã bao gồm ship)
                paymentMethod,
